...
export default function RSVP() {
  ...
  const handleSubmitRSVP = async (e) => {
    ...
    try {
      const companionNames = guests.slice(1).map(g => g.name.trim());
      
      const res = await api.guests.publicRsvp(selectedFamily.id, {
        status: "confirmed",
        companionNames: companionNames,
        email: email,
        favoriteSong: musicSuggestion,
      ...
      console.error(err);
      setError(err.message || "Erro ao confirmar presença. Tente novamente.");
    } finally {
      setLoading(false);
    }
  };
                        ...
                        >
                          <div className="flex items-center gap-3">
                             ...
                             <input
                              ...
                              placeholder={index === 0 ? "NOME DO CONVIDADO PRINCIPAL" : "NOME COMPLETO DO ACOMPANHANTE"}
                              ...
                              required
                              disabled={index === 0} // Don't let them change the main guest name since it is verified
                            />
                          </div>
